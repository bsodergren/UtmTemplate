...
namespace UTMTemplate\Bundle\Carousel;

use UTMTemplate\Bundle\Carousel\Helper\CssMinifier;
use UTMTemplate\Bundle\Carousel\Helper\JsMinifier;
use UTMTemplate\Bundle\Carousel\JavaScript\CarouselAPI;
use UTMTemplate\Bundle\Carousel\JavaScript\CarouselInstance;
use UTMTemplate\Bundle\Carousel\Translator\ArrayTranslator;
use UTMTemplate\Bundle\Carousel\Translator\TranslatorInterface;
use UTMTemplate\Bundle\Carousel\Validator\UrlValidator;
...
class CarouselRenderer
{
    ...
    public function renderHtml(): string
    {
        ...
        if (($options['showDots'] ?? true) && \count($items) > 1) {
            ...
            foreach ($items as $index => $item) {
                $html .= '<button class="carousel-dot'.(0 === $index ? ' active' : '').'" role="tab" aria-label="'.$this->escape($this->translator->translate('go_to_slide', null, ['index' => $index + 1])).'" data-slide="'.$index.'" type="button"></button>';
            }
            $html .= '</div>';
        }

        // Thumbnails (for gallery type)
        if (($options['showThumbnails'] ?? false) && Carousel::TYPE_GALLERY === $type) {
            $html .= '<div class="carousel-thumbnails">';
            foreach ($items as $index => $item) {
                $html .= '<button class="carousel-thumbnail'.(0 === $index ? ' active' : '').'" data-slide="'.$index.'" type="button">';
                if ($item->image) {
    ...
    private function renderCardItem(CarouselItem $item, int $index): string
    {
        ...
        if ($item->hasImageSourceSet()) {
            ...
            $html .= '<div class="carousel-card-image">';
            if ($item->link) {
                $html .= '<a href="'.UrlValidator::sanitize($item->link).'" class="carousel-card-link">';
            }
            if ($shouldLazyLoad) {
...
CSS;
    ...
    private function getImageCss(string $cssId): string
    {
        ...
        $minHeight = ('auto' === $height) ? '400px' : $height;
