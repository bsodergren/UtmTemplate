...
public static $TemplateDir = '/elements/GridView';
    ...
    /**
     * the internal array of \UTMTemplate\Bundle\GridView\Columns added to the table.
     *
     * @var array
     */
    protected $columns = [];
     ...
     * @var array
     */
    public $itemsPerPage = [10, 20, 50, 100];
  ...
  protected $templateDir = '';
    ...
    public function getSortUrl($column)
    {
        ...
        $qs = array_merge($_GET, ['sort' => $column->sortableName, 'sort_dir' => $this->sortDirection]);

        $query = http_build_query($qs);

        if (str_contains($this->sortUrl, '?')) {
    ...
    public function getSortDirectionForColumn($column)
    {
        ...
        if (isset($_GET['sort']) && $_GET['sort'] == $column->sortableName) {
            $sortDirection = (isset($_GET['sort_dir']) && self::SORT_DIRECTION_ASC == $_GET['sort_dir'])
    ...
    public function addColumn($column)
    {
        ...
        if (!$this->noFilters && false != $this->visibleColumns && !\in_array($column->name, $this->visibleColumns)) {
...
<?php
    ...
    public function renderItemsPerPage()
    {
        ...
        ?>
<div class="row">
    <div class="pull-right col-md-12"> Showing
        <?php echo \count($this->dataSource); ?>
        items. <?php
...
<script>
    jQuery(function() {
        $("#grid-view-<?php echo $this->itemsPerPageIdentifier; ?>")
            .change(function() {
                var values = $(
                    '#<?php echo $this->id; ?> .grid-view-filters :input'
                    ).serialize();
                window.location = window.location.pathname + '?' +
                    values +
                    "&<?php echo $this->itemsPerPageIdentifier; ?>=" +
                    $(this).val();
            });
    });
</script>
...
<?php
     ...
     * it will add a UTMTemplate\Bundle\GridView\Columns\ButtonColumn column to the end of the $columns array.
     *
     * @return string
     */
    public function render()
    {
        ...
        if (!empty($this->buttons)) {
            $column = new Columns\ButtonColumn(['buttons' => $this->buttons]);
        ...
        if ($this->showItemsPerPageHeader) {
            echo $this->renderItemsPerPage();
        }

        if ($this->useModalFilters) {
            echo $this->renderModalFilters();
        }
        ?>
...
<tr>
    <td
        colspan="<?php echo \count($this->headers); ?>">
...
<?php
    ...
    public function renderTableRow($data, $index)
    {
        ...
        ?>
<tr
    class="<?php echo $this->getTableRowCss($data, $index); ?>">
...
<?php return ob_get_clean();
    ...
    {
        if ($this->useJqueryJavascripts && true == $this->useColumnFilters) {
...
<script>
    jQuery(function() {
        $('#<?php echo $this->id; ?> .grid-view-filters :input')
            .not('.grid-view-select-all').blur(function() {
